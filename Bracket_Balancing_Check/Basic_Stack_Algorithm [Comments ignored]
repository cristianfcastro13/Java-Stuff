...
/**
 * BracketsBalancing(with comments ignored) class
 * @author CCastro
 */
public class BracketsBalancingComments {
	...
	/**
	 * bracketType - calculates the bracket type
	...
	public static void main(String[] arguments_or_args_for_short) {
		try {
			...
			for(int q = 0; q < arguments_or_args_for_short.length; q++) {
				Scanner inputFile = new Scanner (new File(arguments_or_args_for_short[q])); // Scanner gets its file name from the command-line interface.
				...
				while(inputFile.hasNextLine() & balanced == true) {
					...
					for(int i = 0; i < lineParsed.length; i++) {
						...
						// This if statement is meant to filter one-line comments by checking for double slashes and skipping the entire line from parsing
						if (inLine.length() > 2) {
							commentTemp = inLine.trim();
							commentTemp = commentTemp.substring(0, 2);
						...
						// This if-else-if loop opens and closes multi-line comments when detected
						if (commentTemp.equals("/*") && activeComment != true) {
							activeComment = true;
							break;
						}
						else if (activeComment == true && commentTemp.equals("*/")) {
							activeComment = false;
							break;
						}
						
						// This if loop skips every line while the multi-line comment is open
						if (activeComment == true) {
							break;
						}
						
						// Balancing parsing starts here
